...
/*
카이로스153 생기부 분석기 v11.2  대표 컨설턴트: 신지은
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[v11.2 변경사항]
...
function buildJongRecs(grade, sebu, gyeyeol, gender, majorGroup, achieveBonus=0){
  ...
  const scored = filtered.map(r => {
    ...
    return {...r, tier, ng: Math.round(ng*100)/100, diff: g-r.c, majorMatch, poss};
  }).filter(Boolean);
...
function Result({d, onReset, onReanalyze, isReanalyzing}){
  ...
  const jongRecRecs=buildJongRecs(d.facts.gradeAll,d.facts.sebuLevel,d.계열,d.gender,d.recMajorGroup,achieveBonus);
  const jongWishRecs=buildJongRecs(d.facts.gradeAll,d.facts.sebuLevel,d.계열,d.gender,wishMajorGroup,achieveBonus);
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Noto Sans KR','Apple SD Gothic Neo',sans-serif",color:C.text}}>
      ...
      <div style={{background:C.surface,borderBottom:"1px solid "+C.border,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          ...
          <div style={{fontSize:"10px",color:C.muted}}>카이로스153 · 신지은 · v11.2</div>
        </div>
        ...
        {phase==="upload"&&(
          <>
            ...
            <div style={{background:C.aLight,border:"1px solid "+C.aBorder,borderRadius:"10px",padding:"12px",fontSize:"11px",color:C.accent,lineHeight:1.8,marginBottom:"10px"}}>
              <div style={{fontWeight:700,marginBottom:"3px"}}>v11.2 — nGap 정규화 · 배지 6종 · 맥에듀테크 DB</div>
