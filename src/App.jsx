...
// ── ParsingPhase: 내신 파싱 확인/정정 단계 (v11.4 신규) ──────────
...
function buildJongRecs(grade, docGrade, schoolType, gyeyeol, gender, majorGroup, strength){
  ...
  const adjGrade = calcAdjGrade(parseFloat(grade), schoolType||"일반고", docGrade||"B");
  ...
  const scored = filtered.map(r=>{
    ...
    const poss = calcJongPoss(adjGrade, docGrade||"B", r.c, r.c70);
    const ew = typeof getEvalWeight === "function" ? getEvalWeight(r.u, r.t) : null;
    return {...r, tier, adjGrade, diff: adjGrade-r.c, majorMatch, poss, spreadBadge, verdict, ew};
  }).filter(Boolean);
  ...
  // majorGroup 있을 때 → 매칭 학과만 표시
  const display = majorGroup
    ? Object.values(byUni).filter(r=>r.majorMatch)
    : Object.values(byUni);
  const all = display.sort((a,b)=>
    (to[a.tier]||3)-(to[b.tier]||3)||b.poss-a.poss||a.c-b.c
  );
  return {
...
function buildGyogwaRecs(grade, gyeyeol, gender, majorGroup){
  ...
  const g = parseFloat(grade);  // 교과는 원내신 그대로
  const filtered = db.filter(r=>
    !(gender==="남"&&WOMENS_UNIV.has(r.u)) &&
  ...
  const scored = filtered.map(r=>{
    ...
    return {...r, tier, diff:g-r.c, majorMatch, poss};
  }).filter(Boolean);
...
function Result({d, onReset, onReanalyze, isReanalyzing}){
  ...
  const copyReport=()=>{
    const L=[];
    L.push("KAIROS 153 · 카이로스153 대입컨설팅 · 2027학년도 · v11.4");
    ...
    if(navigator.clipboard && window.isSecureContext){
      ...
      try{
        ...
        if(ok) alert("리포트 복사 완료!");
        else setReportText(text);
    ...
    <div>
      ...
      <div style={{background:C.surface,border:"1px solid "+C.border,borderRadius:"12px",padding:"14px",marginBottom:"10px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:"8px",marginBottom:"8px"}}>
          <div>
            <div style={{color:C.muted,fontSize:"10px",marginBottom:"2px"}}>KAIROS 153 · 신지은 · v11.4</div>
            ...
            {reportText&&(
              <div style={{marginTop:"10px"}}>
                ...
                <button onClick={()=>setReportText("")}
                  style={{marginTop:4,padding:"4px 12px",fontSize:"11px",cursor:"pointer",fontFamily:"inherit",background:"transparent",border:"1px solid "+C.border,borderRadius:"6px",color:C.muted}}>닫기</button>
              </div>
            )}
          </div>
...
export default function App(){
  ...
  const handleParsingConfirm=useCallback(async(gradeInfo,text)=>{
    ...
    try{
      setStep(4);const raw=await callAI(makePrompt1(text,gradeInfo,preInput));
      setStep(6);const data=parseAnalysis(raw,text,gradeInfo);
      // v11.4: adjGrade 기반 scores 재계산 (spreadBadge 균형형 기본)
      if(!data.scores){
        const adj=calcAdjGrade(parseFloat(data.facts.gradeAll),data.schoolType,data.docGrade);
        const sc=calcScoresV114(adj,data.docGrade,data.careerConsistency,data.leadership,data.bongsa,data.hyeobup,data.seongshil,"🟡");
        data.adjGrade=adj;data.scores=sc;data.total=calcTotal(sc);
        data.strength=getStrength(data.total);data.gradeLabel=gradeLabel(data.total);
      }
        ...
        {phase==="upload"&&(
          <>
            ...
            <div style={{background:C.aLight,border:"1px solid "+C.aBorder,borderRadius:"10px",padding:"12px",fontSize:"11px",color:C.accent,lineHeight:1.8,marginBottom:"10px"}}>
              <div style={{fontWeight:700,marginBottom:"3px"}}>v11.4 — 파싱 확인 · 5개년 지표 · 배지 7종 · 26개 전공</div>
