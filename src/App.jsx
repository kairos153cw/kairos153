...
function parseGradesFromText(text){
  ...
  for(let i=0;i<lines.length-4;i++){
    ...
    // 학점수 앞줄이 숫자가 아닌 텍스트여야 함 (과목 구분)
    const prevLine=(i>0?lines[i-1].trim():"");
    if(/^\d+$/.test(prevLine)) continue; // 연속 숫자면 스킵
    for(let j=i+1;j<Math.min(i+12,lines.length);j++){
      const t=lines[j].trim();
      // "A(257)" "B(246)" 형태 (나이스 TXT)
      if(/^[A-E]\(\d+\)$/.test(t)){
        if(j+1<lines.length){
...
function ManualInput({onAnalyze}){
  ...
  const go=()=>{
    ...
    // v11.4: 수동입력도 조정내신 변환 (일반고 기준)
    const adjGradeM=calcAdjGrade(parseFloat(grade)||3.0,"일반고","B");
    const spreadBadgeM="🟡";
