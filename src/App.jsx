...
// ── 학종 구간 판정 ─────────────────────────────────────────────────
// schoolCorr: 특목/자사고만 적용 (일반고 기본값 0) → tier 판정에만 사용
// gap 기반 alpha: 촘촘형(gap<0.3)→0.1 / 일반형→0.3 / 분산형(gap>0.7)→0.5
// 안정 조건 최우선 (구간 무관)
function calcJongTier(g, c, c70, schoolCorr=0){
  const gT=g-schoolCorr;
  const gap=c70-c;
  const alpha=gap<0.3?0.1:gap<=0.7?0.3:0.5;
  if(gT<=c-0.1) return "안정";
  if(gT<=1.9){
    const c30=Math.max(1.0,c-(c70-c)*0.5);
    if(gT<=c30+0.1)   return "안정";
    if(gT<=c+0.5)     return "적정";
    if(gT<=c70+alpha) return "소신";
    return "상향";
  }
  if(gT<=c+0.3)     return "적정";
  if(gT<=c70+alpha) return "소신";
  return "상향";
}
// gap 촘촘도 반환 (UniCard 경고 배지용)
...
// ── buildJongRecs: 학종 추천 ───────────────────────────────────────
// schoolCorr: tier 판정용만 (calcJongTier에 전달), poss/정렬은 원래 등급 사용
function buildJongRecs(grade, sebu, gyeyeol, gender, majorGroup, schoolCorr=0){
  const db=(DB_V11.학종||{})[gyeyeol]||[];
  const g=parseFloat(grade);
  const filtered=db.filter(r=>!(gender==="남"&&WOMENS_UNIV.has(r.u)));
  const scored=filtered.map(r=>{
    const poss=calcJongPoss(grade,sebu,r.c,r.c70);           // 원래 등급 기준
    const tier=calcJongTier(g,r.c,r.c70,schoolCorr);         // schoolCorr → tier만
  ...
  const all=Object.values(byUni).sort((a,b)=>
    ...
    b.poss-a.poss||   // poss 높은 순 (세특/전공매칭 반영)
    a.c-b.c           // 입결 좋은 순 (같은 poss일 때)
  );
  const 안정Max=g>=4.0?3:5;
  const 적정Max=g>=3.5?8:10;  // 3.5이상 8개 제한 (26개 초과 방지)
...
function buildGyogwaRecs(grade, gyeyeol, gender){
  ...
  const scored=filtered.map(r=>{
    ...
    const tier=calcJongTier(g,r.c,r.c70);  // c30 제거, schoolCorr 미적용
    const gapType=getGapType(r.c,r.c70);
...
function UniCard({r, i, majorGroup, mode}){
  ...
  const isTight=r.gapType==="촘촘";  // gap < 0.3: 합격 구간 매우 좁음
  return(
    <div style={{borderBottom:"1px solid "+C.border,padding:"10px 0"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"3px"}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          {isTight&&<span style={{marginLeft:4,fontSize:"9px",background:"#fef9c3",color:"#854d0e",padding:"1px 5px",borderRadius:"4px",fontWeight:700}}>⚠️ 구간좁음</span>}
        </div>
      ...
      {isTight&&<div style={{fontSize:"10px",color:"#854d0e",marginTop:"3px",background:"#fef9c3",borderRadius:"6px",padding:"3px 8px"}}>
        ⚠️ 합격 구간 매우 좁음 (50%컷-70%컷 차이 0.3 미만) — 내신이 컷에 근접해야 유리
      </div>}
...
function Result({d, onReset, onReanalyze, isReanalyzing}){
  ...
  const[schoolCorr,setSchoolCorr]=useState(0);  // 학교군 보정값

  // 대학 추천 계산 (schoolCorr → tier 판정용만)
  const wishMajorGroup=d.altMajor?detectMajorGroup(d.altMajor)||d.wishMajorGroup:d.wishMajorGroup;
  const jongRecRecs=buildJongRecs(d.facts.gradeAll,d.facts.sebuLevel,d.계열,d.gender,d.recMajorGroup,schoolCorr);
  const jongWishRecs=buildJongRecs(d.facts.gradeAll,d.facts.sebuLevel,d.계열,d.gender,wishMajorGroup,schoolCorr);
    ...
    <div>
      ...
      {tab==="recs"&&(
        <div>
          ...
          {recMode==="학종"&&(
            <div>
              {/* 학교군 보정 드롭다운 (특목/자사고 감지 시만 표시) */}
              {d.schoolType==="특목자사고"&&(
                <div style={{marginBottom:"10px",padding:"10px",background:C.roseLight,border:"1px solid "+C.roseBorder,borderRadius:"10px"}}>
                  <div style={{fontSize:"11px",fontWeight:700,color:C.rose,marginBottom:"6px"}}>⚠️ 특목/자사고 감지 — 학교군 보정 (tier 판정에만 적용)</div>
                  <select value={schoolCorr} onChange={e=>setSchoolCorr(Number(e.target.value))}
                    style={{width:"100%",padding:"8px",borderRadius:"8px",border:"1px solid "+C.border,fontSize:"12px",background:C.surface,color:C.text,fontFamily:"inherit"}}>
                    <option value={0}>미적용 (일반고 기준)</option>
                    <option value={0.3}>자사고 중위 (−0.3등급)</option>
                    <option value={0.5}>자사고/특목고 상위 (−0.5등급)</option>
                    <option value={0.7}>최상위 특목고 (−0.7등급)</option>
                  </select>
                  {schoolCorr>0&&<div style={{fontSize:"10px",color:C.sub,marginTop:"5px",padding:"4px 8px",background:C.panel,borderRadius:"6px"}}>
                    내신 {d.facts.gradeAll}등급 → 보정 후 {(parseFloat(d.facts.gradeAll)-schoolCorr).toFixed(2)}등급 기준으로 tier 판정 | 합격가능성 점수는 원래 등급 유지
                  </div>}
                </div>
              )}
